...
\section{Introduction}
 ...
 It is based on the Curry-Howard isomorphism, that is, the logical constructions are instances of more general constructions on types.
 ...
 It forms the base of the computer proof assistants \coq, \matita and \agda.
...
%  TODO: fill in a picture of a tree
 ...
 Many objects in mathematics and logic can be represented as \emph{trees}.
 For instance, a natural number can be considered as such a tree: the possible nodes are $S$---a node with one subtree---and $Z$---a node without subtrees, i.e.\ a \emph{leaf}.
 The natural number $2$ is represented by the tree $S\longrightarrow S\longrightarrow Z$. 
 
%  TODO: finite vs infinite
...
\begin{Long}
 \subsection{Semantics of inductive types}
\end{Long}
\begin{Short}
 \paragraph*{Semantics of inductive types}
\end{Short}
 
 Before turning to \emph{co}inductive types, we give an overview over semantics for inductive types.
 
 Informally, we think of an inductive object as a  tree, consisting of a \emph{root}, 
 to which a number of \emph{subtrees} are attached. The subtrees are themselves trees.
 ...
 By \enquote{semantics of inductive types} we mean a mathematical description of the set/type of trees of a given shape. We are interested in 
 \emph{category-theoretic} characterizations of such sets/types. 
...
\end{Short}
  ...
  Coinductive sets/types describe potentially \emph{infinite} trees, that is, trees that may have an infinite chain of subtrees.
  An example of such a coinductive set/type is given by the \emph{co}natural numbers, which consist of the natural numbers and an additional,
 ...
 dually to inductive sets, \emph{co}inductive sets are characterized as terminal coalgebras of 
...
% TODO: talk about type families

Instead of considering types of trees, we consider, more generally, \emph{families} $T(X)$ of types of trees, families parametrized by a type $X$.
We consider two different uses for this parameter: firstly, the \emph{leaves} of a tree $t:T(X)$ may come from $X$. 
Secondly, all the \emph{nodes} come from the type $X$.

We first consider the first case of \emph{leaf-labeled trees}.
 Let $T(X)$ be a type family of trees with \emph{leaves from $X$}, i.e., $t: T(X)$ is a tree (of a given shape) with leaves in $X$.
 ...
 A well-studied example is that of the lambda calculus, the terms of which can be formalized as leaf-labeled trees over a type of \emph{free variables} \parencite{alt_reus}.
 ...
 If we consider infinite node-labeled trees in IMLTT, however, the algebraic laws of cosubstitution for such trees have to be considered 
 ...
 we thus define a more complicated category of \enquote{models} of the signature specifying those types---coalgebras with extra structure accounting for a c
 omonadic operation.
...
\subsection{Homogeneous and heterogeneous trees}

In this work we consider families of types of (infinite) node-labeled trees. 
Here, \emph{family} refers to the fact that these types are parametrized by a type variable (and hence constitute not a single type, but an infinite family of types), 
from which the nodes of the trees are chosen. We call a tree a \enquote{tree over type $X$} if its nodes are elements of $X$.
A tree over $X$ is called \emph{homogeneous} if its subtrees are again trees over $X$.
...
\begin{definition}%[Category in IMLTT]
 \label{def:cat_imltt}
...
\begin{definition}%[Some categories]
...
compatible with the destructors (elimination); it is the appropriate notion of sameness on inhabitants of these types \parencite{DBLP:conf/types/Coquand93}.
...
\begin{example}\label{ex_stream}
  Let $A$ be a type. The type---setoid, actually---$\stream A$ of \emph{streams over $A$} is coinductively defined via the destructors 
...
\end{Short}
This change of parameter (from $A$ to $E \times A$) in the type of $\tail$ is why the family $\Tri$ is called \enquote{heterogeneous}.
...
\begin{remark}
 ...
 Categorically, the lack of such a computation rule amounts to saying that bisimilarity is a \emph{weakly} terminal bisimulation on the respective codata. (Here, a bisimulation is a binary relation on the codata type that is compatible with the destructors of the codata types.)
 In the setting of Homotopy Type Theory \parencite{hottbook}, where one would identify bisimilarity with propositional equality, a proof-relevant treatment would be adequate.
\end{remark}
...
\begin{definition}%[Relative comonad]
 \label{def:rel_comonad}
  ...
  \begin{packitem}\setlength{\itemsep}{\itemizedist}
...
Apart from the implementation of our codata types via axioms, the mechanization does \emph{not} rely on any additional axioms.
