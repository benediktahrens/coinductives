...
It is the purpose of this section to define this extra structure.
...
\begin{example}[$\cut$ for $\Tri$]
  The relative comonad $\Tri$ from \Cref{ex:tri_comonad} comes with a $\cut$ operation as in \Cref{def:rel_comonad_with_cut},
...
\begin{definition}[Product in context]\label{def:product_in_context}
 Suppose $F:\C\to\D$ strong monoidal as in \Cref{def:extend}, and $T$ a comonad relative to $F$ with a $\cut$ operation 
...
\begin{theorem}[Coinitial semantics for triangular matrices with redecoration]\label{ex:final_sem_tri}
   ...
   Then the pair $(\Tri, \tail)$ consisting of the relative comonad $\Tri$ of \Cref{ex:tri_comonad} together with 
