...
Let $E$ be an arbitrary fixed type.
   Infinite triangular matrices are given by the following coinductive definition in pseudo-\textsf{Coq} syntax: 
  \begin{lstlisting}
Coinductive Tri (E : Set) : Set -> Set :=
   head : forall A : Set, Tri (A) -> A.
 | tail : forall A : Set, Tri (A) -> Tri (E x A).
  \end{lstlisting}
%TODO: that's shitty. should have inference rules for destructors here, not coq code
%TODO: use bussproofs to put inference rules
