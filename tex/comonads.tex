...
\section{Introduction}
 ...
 In a type-theoretic setting as given by Martin-L\"of type theory \parencite{martin_lof}, a similar characterisation is possible, but some attention has to be given to 
...
%  
 ...
 It forms the base of two important computer proof assistants, \coq and \agda.
 \textsf{W}-types in \emph{Homotopy Type Theory} (HoTT), an extension of intensional Martin-L\"of type theory, are studied by \textcite{DBLP:conf/lics/AwodeyGS12}.
 ...
 Dually to inductive sets, \fat{co}inductive sets such as streams are characterised as terminal objects, e.g., in the work of \textcite{jacobs1997tutorial}.
 Inhabitants of such sets are equal if and only if they are \emph{bisimilar} (see \textcite{DBLP:journals/mscs/TuriR98}):
 Intuitively, two elements of a coinductive set are the same if they allow for the same observations.
 
 This correspondence between equality and bisimilarity fails in MLTT, when equality is considered to be given by the 
 ...
 Our terminal semantics result characterises not only the codata types themselves but also the bisimilarity relation and
...
%  
%  In the present work, we make use of the dual notion of \emph{relative comonad} and its companion notion of \emph{comodule over relative comonads} to capture the properties of cosubstitution on the considered codata types.
...
%  For this, we develop the notion of \emph{comodule over a relative comonad}: \emph{morphisms of comodules} capture the interplay between 
...
Maps into a coinductive data type are specified by the observations, i.e.\ the value of the destructors, on the output of those maps, as illustrated in \Cref{eq:tail_sredec}.
...
%  
 ...
 Given a matrix over type $A$, its $\tail$ can be considered as a trapezium as indicated by the green line in \Cref{fig_tri}, or alternatively, as
...
\begin{Short}
 Under some conditions on the functor $F$, one can obtain comonads relative to $F$ from (absolute) comonads on the 
 source category $\C$. Details are given in the long version of this article.
\end{Short}
...
A morphism of relative comonads $\diagonal : \Tri \to \stream$ is defined with carrier
 $\diagonal_A : \Tri A \to \stream A$ via the following clauses:
...
\begin{Short}
 Similarly to relative comonads, comodules over these are functorial. We omit the details.
\end{Short}
...
As presented in \Cref{def:tautological_comodule}, every relative comonad induces a comodule over itself, the tautological comodule.
...
%  While it is obvious how to define a general notion of signature and a category of coalgebras for any such signature,
% we refrain from doing so for lack of further examples.
...
\begin{definition}[Coalgebras for $\stream$]\label{cat_stream}
  Inspired by \Cref{ex_stream_comonad} and \Cref{ex_tail_comodule}, we define a \fat{coalgebra for $\stream$} to be given by a pair $(S,t)$ 
  consisting of
  \begin{itemize}
...
This example is a reformulation of \Cref{ex_tail_comodule}.
...
one could hope to reflect bisimilarity into propositional equality by \emph{quotienting},
...
Finally, we would like to integrate \emph{equations} into the notion of signature, equations which 
