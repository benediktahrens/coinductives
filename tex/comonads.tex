...
\end{Long}
\begin{Short}
 Given a matrix over type $A$, its $\tail$ can be considered as a trapezium or, alternatively, as a triangular matrix over type $E\times A$, 
...
Every relative comonad comes with a canonical comodule over it:
...
\begin{definition}[Coalgebras for $\stream$]\label{cat_stream}
  Inspired by \Cref{ex_stream_comonad} and \Cref{ex_tail_comodule}, we define a \fat{coalgebra for $\stream$} as a pair $(S,t)$ 
  consisting of
  \begin{itemize}
