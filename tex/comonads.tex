...
\section{Introduction}
 ...
 The second one is the \emph{heterogeneous} codata type family of infinite triangular matrices parametrized by the type of diagonal entries.
  ...
  \Cref{sec:computer_checked,sec:type_vs_set,sec:organisation} of the introduction concern more \enquote{administrative} aspects.
...
We first consider the first case of \emph{leaf-labeled trees}.
 ...
 \subsection{Organisation of the paper} \label{sec:organisation}
  ...
  In \Cref{sec:formal} we give an overview of the formalization of this work in the proof assistant \coq.
...
% 
  \begin{align} \comp{\redec~f}{\head} := f \quad\text{ and } \quad
    ...
    \end{align}
An illustration of the redecoration operation is given in \Cref{fig:redec} (taken from \cite{DBLP:conf/types/MatthesP11}).    
\begin{figure}[bt]
  \centering
  \begin{tikzpicture}[scale = 0.6]
...
Here, the need for the family of auxiliary functions 
  ...
  An illustration of this operation is given in \Cref{fig:lift} (taken from \cite{DBLP:conf/types/MatthesP11}).    
  \begin{figure}[bt]
  \centering
  \begin{tikzpicture}[scale = 0.6]
    \foreach \y in {0,...,2}
...
The map $\diagonal_A$ is defined via the clauses
 ...
 \end{equation*}
Instead of proving the comonad morphism axioms for this map, we will, in \Cref{ex_diag_terminal}, specify the same map via a universal property. 
...
In \Cref{sec:coalgebras_for_tri} we generalize \Cref{ex_tri_prod_comod}---precomposition with a product---to more general relative comonads over suitable categories.
...
\end{defn}
% 
We conclude the section with some constructions of comodules, which are not used in the following.

\begin{lem}
Let $T$ be a comonad relative to $F : \C \to \D$, and let $M : \C \to \E$ be a comodule over $T$.
Let $G : \E \to \X$ be a functor.
Then $G \circ M : \C \to \X$ is a comodule over $T$.
...
Then $M \circ G : \C \to \E$ is a comodule over $T$.
...
Then $M + N, c \mapsto M c + N c$ is a comodule over $T$.
...
namely that of a coalgebra of some endofunctor. The relationship between models as in \Cref{cat_stream} and coalgebras is as follows:
...
More precisely, \Cref{thm_stream_terminal} says that the rules given in \Cref{stream_rules} allow to prove that
...
\begin{ex}\label{ex_diag_terminal}
  ...
  which has, as underlying morphism of relative comonads, the one defined in \Cref{ex_diag}. 
...
\begin{rem}[Canonical $\cut$ operation]\label{canonical_cut}
 ...
 It follows from the axioms of comonad morphisms that a comonad morphism $\tau : T\to S$ satisfies the equation of \Cref{def:morphism_comonad_cut} 
...
\begin{rem}[Pushforward commutes with product in context]\label{rem:prod_pullback_commute}
 The constructions of \Cref{def:product_in_context} and \Cref{def:pushforward_comodule} commute:
...
In the formalization we add the rules of \Cref{stream_rules} and \Cref{tri_rules} as axioms to the \coq system, via the \lstinline!Axiom! vernacular. In order to ensure consistency, we 
\begin{enumerate}
...
The correspondences between formalized statements and the statements in this article are given in \Cref{sec:table_formal_informal}.
