...
\begin{abstract}
 ...
 In this work, we study the notions of \emph{relative comonad} and \emph{comodule over a relative comonad}, and exhibit
 natural occurrences of these notions in the definition of category-theoretical semantics of coinductive types in 
...
\section{Introduction}
 ...
 We then use these notions to characterise properties of several coinductive data types and operations on them in intensional Martin-L\"of type theory.
 
 Relative monads were introduced by \textcite{DBLP:conf/fossacs/AltenkirchCU10} as a generalization of the notion of monad,
 to allow for a monadic structure on functors that are not necessarily endo.
 Various data types in type theory can be equipped with the structure of a relative monad, such as 
 lambda terms over finite sets \parencite{DBLP:conf/fossacs/AltenkirchCU10} as a functor from finite sets to sets,
 or lambda terms equipped with a reduction preorder \parencite{ahrens_relmonads} as a functor from sets to preorders.
 
 The purpose of the present work is to present some applications of the dual notion of relative \emph{co}monad in intensional Martin-L\"of 
 type theory (IMLTT). More precisely, we equip some \emph{co}inductive type families with a relative comonadic structure.
 The need to consider \emph{relative} comonads for coinductive data types in IMLTT arises from the fact that the 
 equality relation given by Martin-L\"of's identity type does not yield an adequate notion of \enquote{sameness} for inhabitants of such types.
 ...
 just as plain types.
 For instance, we consider the codata type of streams over a base type $A$ in IMLTT as a functor 
 $\stream : \Set \to \Setoid$.

 
 Furthermore we develop the notion of \emph{comodule over a relative comonad} and use it to characterise the \emph{destructors} of the
 coinductive data types we consider. More precisely, the notion of \emph{morphism of comodules} captures the interplay between 
 the destructors and the canonical \emph{cosubstitution operation} we define on the considered codata types.
 This constitutes a generalization and dualization of work by \textcite{DBLP:journals/iandc/HirschowitzM10},
 who use \emph{modules over (plain) monads} to characterise the constructors of certain inductive data types.
 ...
 but also the bisimilarity relation on those terms, and a canonical cosubstitution operation.
...
%  
 ...
 The category-theoretic concepts studied in this work are agnostic to the foundational system being worked in.
...
Throughout this article, we use type-theoretic notation,  writing $t:T$ to indicate that $t$ is of type $T$. 
 ...
 The techniques we use to characterize this family of coinductive types are dual to those used by \textcite{DBLP:journals/iandc/HirschowitzM10}, 
 in whose work the crucial notions are those of \emph{monads} and \emph{modules over monads}.
 \textcite{DBLP:journals/iandc/HirschowitzM10} give an initial semantics result for languages with variable binding, 
 such as the lambda calculus implemented as a heterogeneous data type. As such, it is based on the work by \textcite{alt_reus},
 ...
 \subsection*{Organisation of the paper}
  ...
  In \Cref{sec:tri} we present the coinductive type family $\Tri$ of infinite triangular matrices and some operations on those
            matrices.
...
The motivating example we refer to throughout the paper concerns coinductive type families in Intensional Martin-L\"of type theory (IMLTT) \parencite{martin_lof}, 
...
\begin{definition}[Some categories]\label{def:set_setoid}
 ...
 We denote by $\Setoid$ the category an object of which is a type equipped with an equivalence relation.
 A morphism between setoids is a type-theoretic function between the underlying types that is compatible with the equivalence relations of the source and target setoids.
...
In this section we present some coinductive data types which serve as examples in the later sections.
We also define \emph{bisimilarity} for each codata type, a coinductive equivalence relation which is considered 
as the appropriate notion of equality on inhabitants of these types.
We denote bisimilar elements using an infix $\sim$, as in $t \sim t'$.
     ...
     \UnaryInfC{$\shead(t) = \shead(t')$}
      \DisplayProof
                        \hspace{3ex}
                                       ...
                                       \DisplayProof   
  \end{center}
  \caption{Destructors and bisimilarity for the coinductive type of streams} \label{fig:tri_destructors}
\end{figure}
    
   We define a cosubstition operation $\sredec_{A,B} : (\stream A \to B) \to \stream A\to\stream B$ on streams via the following equations:
   \begin{align}\shead\bigl(\sredec~f~t\bigr) &:= f~t \quad\text{ and } \notag\\
                     \stail\bigl(\sredec~f~t\bigr) &:= \sredec~f~(\stail~t) \enspace . \label{eq:rest_redec}
    \end{align}
%    TODO: check cosubst for stream
...
This codata type was studied in an article by
 \textcite{DBLP:conf/types/MatthesP11} which is available via open access. 
 The reader is strongly urged to read (the first two sections of) that work 
 before the present one; here we only recall the most important
 definitions. 
 
 
 The codata type family $\Tri$ of infinite triangular matrices we are about to define
...
%      \draw[color=green, dashed]  (2.8,-3.6) -- (3.5,-4.2);
 \end{tikzpicture}
  \caption{An infinite triangular matrix over type $A$, schematically}\label{fig_tri}
 \end{figure}
 
 We define this data type coinductively via two destructors $\head$ and $\tail$, whose types are given in the first line of \Cref{fig:tri_destructors}.
 We also define a coinductive relation---\emph{bisimilarity}---on the inhabitants of that type, via the destructors of the second line of \Cref{fig:tri_destructors}.    
 Here we write $t \sim t'$ for bisimilar matrices $t$ and $t'$.
 This defines a \emph{setoid} $\Tri A$ for any type $A$, given by the type of matrices equipped with bisimilarity as equivalence relation.
 ...
 underlying \emph{type}. However, the two inference rules of \Cref{fig:tri_destructors} are the only place this 
 abuse actually happens. In the following, $\Tri A$ usually denotes the \emph{setoid} of infinite triangular matrices over the type $A$.
...
\end{figure}
  ...
  is defined in \parencite{DBLP:conf/types/MatthesP11}, which is then shown to constitute the cobind
  operation of a \enquote{weak constructive} comonadic structure on the family $\Tri$. It is given through the equations
...
Here, the family of functions 
  ...
  The auxiliary function $\cut_A : \Tri(E\times A) \to \Tri A$ is defined corecursively via
   \begin{align*}\head\bigl(\cut~t\bigr) := \pr_2(\head~t) \quad\text{ and } \quad
      ...
      \end{align*}
%       
All the operations are suitably compatible with the bisimilarity relation, so that they can be equipped with the types
  \begin{align*}
    ...
    \cut_A &:  \Setoid(\Tri(E\times A) , \Tri A)
  \end{align*}
\end{example}
...
\begin{example}
%  TODO: signatures generalizing streams
\end{example}
...
\begin{example}[Streams in MLTT]
  ...
  %TODO: example: streams as relative comonads 
\end{example}
...
of redecoration with the other destructor, $\tail$. 
...
%  TODO: is diag for streams a morphism of comonads?

We define $\diagonal_A : \Tri A \to \stream A$ via the following equations:
...
\begin{example}
   The map $A \mapsto \Tri(E\times A)$ is equipped with the structure of a comodule over the relative comonad $\Tri$ by
   defining the comodule operation $\mcobind$ as
     \[ \mcobind_{A,B} (f) := \redec (\extend~f) \enspace . \]
%  TODO: \Tri(E\times \_) as example of comodule
\end{example}
...
%  TODO: terminality for streams and signatures
...
\begin{theorem}
 The pair $(\stream, \stail)$ is the terminal coalgebra in the category of coalgebras defined in \Cref{cat_stream}.
\end{theorem}
...
\begin{example}
  ...
  morphism of tautological comodules over $\Tri$:
   \[ t^{\Tri} := \comp{\tail}{\cut}  : \Tri \to \Tri \enspace . \] 
  This defines a coalgebra $(\Tri, t^{\Tri})$ and the resulting terminal coalgebra morphism
   $(\Tri, t^{\Tri}) \to (\stream, \stail)$ is the one defined in \Cref{ex_diag}.
\end{example}
...
We now define a category in which the codata type of triangular matrices, equipped with the bisimilarity relation of \Cref{ex:tri_comonad}
