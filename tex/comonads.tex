...
\Copyright{Benedikt Ahrens and R\'egis Spadotti}
\subjclass{F.3.2 Semantics of Programming Languages}
\keywords{relative comonad, Martin-L\"of type theory, coinductive type, computer theorem proving}
% Author macros::end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Editor-only macros:: begin (do not touch as author)%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \serieslogo{}%
    \volumeinfo%
           ...
           {39}% volume number
           {1}%
           {1001}% first page of the article (will be set by the editors)
    \EventShortName{TYPES'14}
    \DOI{10.4230/LIPIcs.TYPES.2014.XXX}% XXX is not the final value!
...
\section{Introduction}
 ...
 postulating the existence of \emph{co}inductive types.
...
type $F_i(X)$ for maps of types $F_i : \Set \to \Set$. Obviously, when the functors $F_i$ are identity functors, we get back homogeneous trees; heterogeneous trees are thus more general than
homogeneous trees.
...
We first consider the  case of \emph{leaf-labeled trees}.
 ...
 This replacement operation is called \emph{(simultaneous) substitution}---\enquote{simultaneous} because all the leaves are 
 substituted at once. 
...
corecursively, the $\stail$ of that list, which is again an infinite list over $A$.
...
\begin{rem}[Canonical $\cut$ operation]\label{canonical_cut}
 ...
 $E:\C_0$ satisfying the properties of Definition \ref{def:rel_comonad_with_cut} by setting
   $ \ccut_A := \cut_A := T\bigl(\pr_2(E,A)\bigr)$.
...
In the formalization, we axiomatize the considered coinductive type families as given by the rules of Figures \ref{stream_rules} and \ref{tri_rules}. 
In order to ensure consistency, we 
\begin{enumerate}
 \item use \coq \emph{module} types to encapsulate the axioms, implemented via the \lstinline!Axiom! vernacular,  and
...
Our results are hence axiom-free with respect to the theory implemented in the \coq proof assistant.
...
The \coq source files  are available from the project web site \parencite{trimat_coq}.
...
\end{table}
 ...
 Version 8.5 of \coq~introduces a new, optional, universe polymorphism \cite{DBLP:conf/itp/SozeauT14}.
 In our code repository \cite{trimat_coq}, we provide a version of our code for \coq~8.4pl6 and two versions for \coq~8.5 (beta2 at the time of writing);
 one where universe polymorphism is not employed, and where hence the aforementioned duplication of code is necessary, and one where 
 polymorphism is used to get rid of that duplication.
 However, in that latter version, getting our files accepted by \coq~then required giving up on canonical structures, which do not seem to play well (yet) with universe polymorphism.
