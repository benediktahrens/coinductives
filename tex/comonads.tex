...
\section{Introduction}
 ...
 We thus consider such codata types as a \emph{setoid}, that is, a type equipped with an equivalence relation---bisimilarity---rather than 
 just a plain type.
 For instance, we consider the codata type of streams over a base type $A$ in IMLTT as a functor 
 $\stream : \Set \to \Setoid$.

 
 We furthermore develop the notion of \emph{comodule over a relative comonad} and use it to characterise the \emph{destructors} of the
 coinductive data types we consider. This constitutes a generalization and dualization of work by \textcite{DBLP:journals/iandc/HirschowitzM10},
...
The motivating example we refer to throughout the paper concerns a coinductive type family in Intensional Martin-L\"of type theory (IMLTT), 
...
\begin{definition}[Some categories]\label{def:set_setoid}
 We denote by $\Set$ the category of types and total functions. 
...
Here, the family of functions 
  ...
  Note that all the operations are suitably compatible with the bisimilarity relation, so that they can be equipped with the types
  \begin{align*}
...
\begin{example}[Streams in MLTT]
  ...
  \end{itemize}
  We refrain from showing the proof of the comonadic laws here; they can be found in our \coq library.  
...
\begin{example}
%  TODO: is zip for streams a morphism of comonads?
\end{example}
