...
\begin{definition}\label{def:eq}
 The functor $\eq : \Set\to\Setoid$ is defined as the left adjoint to the forgetful functor 
  $U : \Setoid \to \Set$.
  Explicitly, the functor $\eq$ sends any set $X$ to the setoid $(X,=_X)$ given by the set $X$ itself, equiped
...
\end{definition}
 ...
 We leave it up to the reader to find counterexamples for the comonad law involving $\cobind$.

For heterogeneous inductive types, \textcite{DBLP:journals/iandc/HirschowitzM10} solve an analogue question by considering the notion of
...
% TODO: reformulate def of morphism of comonads with cut using the diagram for rest
...
We now have all the ingredients to state (and prove) our main theorem, obtained by collecting all the information we have gathered so
far about $\Tri$, and abstracting from this particular comonad:
...
\subsection*{Acknowledgments}
 We thank Ralph Matthes and Paige North for many helpful discussions.
