...
\begin{remark}[$\tail$ is not a morphism of comonads]
  Let $T$ be a comonad with cut relative to a strong monoidal functor $F:\C\to\D$ between cartesian monoidal categories,
 ...
 counit is defined as
  ...
  and the cobind operation as
   \[ \cobind^{T(E\times \_)}_{A,B}(f) :=  \cobind^T(\extend~f)\]
  with $\extend$ defined as in \Cref{def:rel_comonad_with_cut}.
