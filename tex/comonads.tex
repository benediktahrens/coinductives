...
\begin{remark}[Coalgebras for stream]
   Traditionally, streams over a fixed set $A$ are described as terminal coalgebra of the 
   endofunctor $S_A : X \mapsto A \times X$.
   Our category of models for stream differs from the category of coalgebras of $S_A$ in that 
   a model of \Cref{cat_stream} models the \emph{family of streams with varying base type}, rather than streams over a fixed base type.
   
   Coalgebras modeling the family of streams could be obtained for sets by considering a higher-order endofunctor
   \[ S : [\Ens, \Ens] \to [\Ens,\Ens], \enspace S(X)(A) := A \times X(A) \enspace . \]

   The reason for us to consider the whole family of streams rather than streams over a fixed base type is 
   that our notion model in terms of (relative) comonad incorporates a notion of \emph{cosubstitution} as in \Cref{eq:tail_sredec}.
   Such a cosubstitution is given by a family of maps between streams over different base types---we hence need to consider models of the whole 
   family of streams rather than models of streams over a fixed base type.
\end{remark}
