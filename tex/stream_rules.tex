...
\def\fCenter{\mbox{\Large$\entails$}}


               \AxiomC{$R : \stream A \to \stream A \to \Set$} \noLine\UnaryInfC{$x,y : \stream A \entails R~x~y \to \shead~x = \shead~y$} \noLine
                \UnaryInfC{$x,y : \stream A \entails R~x~y \to R~(\stail~x) (\stail~y)$}  %\doubleLine
               \UnaryInfC{$x,y : \stream A \entails R~x~y \to \bisim~x~y$}
               \DisplayProof%
\end{center}
