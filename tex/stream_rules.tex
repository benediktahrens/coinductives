...
\subsection{Bisimilarity}\label{stream_bisim}          
    ...
    \AxiomC{$s, t : \stream A$} \AxiomC{$p: \bisim_A~s~t$} %\doubleLine
     \BinaryInfC{$\shead_A~s = \shead_A~t$}
      \DisplayProof
                        \hspace{3ex}
                                       \AxiomC{$s,t : \stream A$} \AxiomC{$p: \bisim_A~s~t$}%\doubleLine
                                       \BinaryInfC{$\bisim (\stail_A~s) (\stail_A~t)$}
                                       \DisplayProof%
\end{center}
  \item[Creation]\hfill \\                                     
...
\begin{center}
               \AxiomC{$R : \stream A \to \stream A \to \Set$} \noLine\UnaryInfC{$\forall~s,t : \stream A, R~s~t \to \shead~s = \shead~t$} \noLine
                \UnaryInfC{$\forall~s,t : \stream A, R~s~t \to \bisim (\stail~s) (\stail~t)$}  %\doubleLine
