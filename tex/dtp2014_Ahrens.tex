...
\AxiomC{$t : \LC (X+1)$}
 ...
 \end{center}
    ...
    \item <2->[Heterogeneity of $\Abs$:] \hfill \\ recursive argument with bigger parameter $X+1$
    
    \item <3->[Substitution:] \hfill\\        
       $ \subst_{X,Y} : \bigl(X \to \LC Y \bigr) \to \LC X \to \LC Y $
    
    \item <3->[Avoiding capture:] \hfill \\ $\shift_{X,Y} : \bigl(X \to \LC Y \bigr) \to X+1 \to \LC(Y+1)$
...
\begin{frame}
 \frametitle{Initial semantics for $\lambda$-calculus using monads}
   \begin{definition}[Model for signature of $\LC$, H \& M '07]
     \begin{itemize}
      \item a monad $(T, \unit, \monbind)$ on $\textsf{Set}$ (or $\Set$)
   ...
   \begin{lemma}[Initial semantics for $\LC$, H \& M '07]
      $(\LC, \App, \Abs)$ is the initial model, where $\LC = (\LC,\Var,\subst)$
   \end{lemma}
...
%                               \RightLabel{Intro}
               \TrinaryInfC{$\coiter_A~hd~tl : T \to \stream A$}
               \DisplayProof%
...
\begin{frame}
 \frametitle{Sameness for streams: bisimilarity}
%   \begin{description}
%    \item []
   
%    \item []
%          \AxiomC{$x, y : \stream A \vdash R : \Set$} 


 Propositional equality of IMLTT not adequate for infinite objects \\
 $\qquad\leadsto$ bisimilarity

~\\~\\


           \AxiomC{$ R~s~t \to \shead~s = \shead~t$}
                \AxiomC{$ R~s~t \to R~(\stail~s) (\stail~t)$}  %\doubleLine
%                 \RightLabel{Bisim-Intro}
               \BinaryInfC{$R~s~t \to s \sim t$}
               \DisplayProof%
               
~\\~\\

\begin{definition}
 Bisimulation := relation compatible with destructors
\end{definition}
...
% \begin{block}{Axioms are equivalent to}
%      existence of terminal coalgebra of
%      \begin{align*}
%            P :  \to \Setoid \\
%                 
%      \end{align*}
...
\begin{frame}
 \frametitle{Cosubstitution on streams}
...
\begin{frame}
 \frametitle{Comonadic structure of streams}
...
\begin{frame}
 \frametitle{About comodules over relative comonads}
  \begin{itemize}\setlength{\itemsep}{1em}
   \item \enquote{Comodule over a (relative) comonad} is generalization of (relative) comonad
   \item Every comonad is a comodule over itself
   \item Analogy (for modules over a monad): module over a ring
  \end{itemize}
...
\begin{frame}
 \frametitle{Terminal semantics for $\stream$}
  ...
  \begin{definition}[Category of models]
...
\begin{frame}
 \frametitle{Cosubstitution: redecoration}
...
\begin{frame}
 \frametitle{$\Tri$ is a weak constructive comonad}
  ...
  \begin{center}
             ...
             \AxiomC{$ R~s~t \to \head~s = \head~t$}
                \AxiomC{$ R~s~t \to R~(\tail~s) (\tail~t)$}  %\doubleLine
%                 \RightLabel{Bisim-Intro}
               \BinaryInfC{$R~s~t \to s \sim t$}
               \DisplayProof%
  ...
  \end{center}
    ...
    Bisimilarity $\sim$ weakly terminal bisimulation.
...
\begin{frame}
 \frametitle{A specified $\cut$ for any model}
  \begin{itemize}
    \item  $\cut$ is not a categorical operation
    \item fix: every model $(T,r)$ comes with a \fat{specified}
       \[ c_A : T (E\times A) \to TA \]
 ...
 \begin{lemma}[for real this time]
  $(\Tri,\cut,\tail)$ is terminal in the category of models $(T,c,r)$.
 \end{lemma}
