...
\begin{definition}%[Coalgebras for $\stream$]
 \label{cat_stream}
  ...
  consisting of
  \begin{itemize}
   \item a function $S : \Set\to\Setoid$  and
   \item a family of functions $t_A : SA \to SA$.
  \end{itemize}
  A coalgebra morphism $(S,t) \to (S',t')$ is given by a family of functions $\tau_A : S \to S'$ such that
     $ \comp{t_A}{\tau_A} = \comp{\tau_A}{t'_A}$.
\end{definition}
