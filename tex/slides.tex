...
\begin{frame}
 ...
 \begin{block}{Extended to some \fat{heterogeneous} inductive data types}
 
  \begin{itemize}
    \item motivation: \fat{binding syntax} modelled through %($\lambda$-calculus)  
                  \fat{heterogeneous} data type
           \[ \LC(X) \enspace ::= \enspace \lambda : \LC(X + 1) \to \LC(X) \enspace | \enspace \ldots \]
    \item Fiore, Plotkin and Turi '99, Hirschowitz and Maggesi '07
...
\begin{frame}
 ...
 \begin{block}{Analogous situtation for syntax:}
   For the lambda calculus with monadic substitution:
      \[ \subst~f~(\lambda~t) = \lambda~(\subst~(\shift~f)~t) \]
 \end{block}
  ...
  \begin{itemize}
   ...
   \item characterize distributivity of substitution over constructors
      \begin{align*} \bind{\App~(s,t)}{f} \quad &= \quad \App~(\bind{s}{f},\bind{t}{f}) \\
                     \bind{\lambda~t}{f}       \quad &= \quad \lambda~(\bind{t}{\shift~f})
      \end{align*}
  \end{itemize}
 \end{block}
 ...
 \frametitle{Technical difficulty: definition of $\lift$}
   \begin{itemize}
    ...
    \item fix: every coalgebra comes with a \fat{specified}
       \[ \cut^T_A : T (E\times A) \to TA \]
          and equations characterizing $\cut^T$
    \item $\cut$ for $\Tri$ uniquely determined by these equations
   \end{itemize}
 \begin{lemma}[for real this time]
  $(\Tri,\cut,\tail)$ is terminal in the category of coalgebras.
 \end{lemma}
