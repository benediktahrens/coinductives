...
In this work, we study the notions of \emph{relative comonad} and \emph{comodule over a relative comonad}.
 We then use these notions to characterize several \emph{co}inductive data types in intensional Martin-L\"of type theory (IMLTT)
 ...
 Consequently, we consider two maps into a coinductive type to be the same if they are \emph{pointwise bisimilar}, thus
 requiring a form of extensionality for such maps.
 ...
 an example of \emph{heterogeneous} codata type, infinite triangular matrices.
 ...
 we develop the notion of \emph{relative comonad}---the dual to \emph{relative monads} \parencite{DBLP:conf/fossacs/AltenkirchCU10}---and \emph{comodule over a relative comonad}.
 ...
 A full article is available on the arXiv \parencite{trimat_coq}.
